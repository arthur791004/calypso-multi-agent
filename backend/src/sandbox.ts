...
const SANDBOX_CLAUDE_MD = [
  ...
  "- **Push** (branch update — PR already exists): `shipyard:sandbox push` with no args.",
  "- **Open a new PR**: before the first push, read `.github/pull_request_template.md` at the repo root if it exists and fill out its sections (Proposed Changes, Why, Testing Instructions, Pre-merge Checklist, ...). Pipe the filled body into the CLI and pass the PR title via `--title`:",
  "",
  "    ```",
  "    printf '%s' \"$pr_body\" | shipyard:sandbox push --title \"short descriptive title\"",
  "    ```",
  "",
  "  If you omit `--title` / stdin body, the backend falls back to `gh pr create --fill`, which uses the last commit message and ignores the template — so only rely on that for throwaway branches.",
